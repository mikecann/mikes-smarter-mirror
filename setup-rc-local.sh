...
# Ensure /etc/rc.local has the proper shebang line and permissions
echo "[INFO] Ensuring /etc/rc.local has the proper shebang line and permissions..."
sudo bash -c "grep -q '^#!/bin/bash' $RC_LOCAL_FILE || echo '#!/bin/bash' > $RC_LOCAL_FILE"
...
sudo bash -c "grep -q 'bun run start:prod' $RC_LOCAL_FILE || sed -i -e '$i \\
